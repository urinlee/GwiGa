...
import { GeneralSettingForm, roomGeneralSetSchema } from "@/schemas/setting/room/schemas";
...
export function RoomGeneralSettingsForm( {roomid, defaultValues}: { roomid: string, defaultValues: GeneralSettingForm }) {
    ...
    const onSubmit = async (data: GeneralSettingForm) => {
        try {
            const response = await fetch(`/api/group/${roomid}/setting/general`, {
                method: "POST",
                headers: {
