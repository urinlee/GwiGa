...
import CreateStatusList from "@/features/group/create/components/CreateStatusList/CreateStatusList";
...
export default function CreateGroupPage() {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<CreateGroupForm>({
        resolver: zodResolver(createGroupSchema),
        defaultValues: { name: "", description: "" },
    });

    // 태그/상태는 hidden input(GroupTag/GroupStatus)으로 렌더링되므로 제출 시 FormData로 함께 읽는다.
    const onSubmit = async (data: CreateGroupForm, event?: BaseSyntheticEvent) => {
        const formData = new FormData(event?.target as HTMLFormElement);
        const tag = formData.getAll("GroupTag") as string[];
        const status = formData.getAll("GroupStatus") as string[];
        ...
        try {
            ...
            router.push(`/group/${result.id}/invite?type=new`);
